...
async def get_user_id(username: str) -> Optional[str]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if user_id:
                _cache.set(cache_key, user_id, ttl_hours=24)
            return user_id
...
async def get_leagues(user_id: str, season: int = CURRENT_SEASON) -> list:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            resp.raise_for_status()
            data = resp.json()
            _cache.set(cache_key, data, ttl_hours=6)
            return data
    except Exception as e:
...
async def get_all_players() -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            resp.raise_for_status()
            data = resp.json()
            _cache.set(cache_key, data, ttl_hours=24)
            return data
    except Exception as e:
...
async def get_trending_players(trend_type: str = "add", limit: int = 25) -> list:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            resp.raise_for_status()
            data = resp.json()
            _cache.set(cache_key, data, ttl_hours=1)
            return data
    except Exception as e:
